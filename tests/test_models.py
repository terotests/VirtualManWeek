...
class TestBasicOperations:
    ...
    def test_upsert_project(self, temp_db):
        ...
        # Verify project was created using list_active_projects
    ...
    def test_insert_time_entry(self, temp_db):
        ...
        # Verify entry was created by checking database directly
        conn = models.connect()
        cursor = conn.cursor()
        cursor.execute("SELECT project_id, mode_label FROM time_entries WHERE project_id = ?", (project_id,))
        row = cursor.fetchone()
        assert row is not None
